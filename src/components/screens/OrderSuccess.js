    ...
    const getOrders = async () => {
      try {
        ...
        if (response.data.orders?.length > 0) {
          ...
          order.services_with_price = JSON.parse(order.services_with_price);
          {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.successMessage}>
        Merci d'avoir confirmé votre rendez-vous. Nous vous contacterons
        bientôt!
      </Text>
      <View style={styles.detailsContainer}>
        ...
        <View>
          ...
          {orderData &&
            orderData?.services_with_price?.map((service, index) => (
          ...
          <View style={styles.tableRow}>
            ...
            <Text style={[styles.tableCell, styles.totalValue]}>
