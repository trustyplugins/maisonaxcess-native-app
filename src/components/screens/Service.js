...
const Service = () => {
  ...
  const [modalVisible, setModalVisible] = useState(
    serviceDetail?.services?.length > 0 ? false : true
  );
  ...
  const calTotalOrder = () => {
    if (customerAddress.services.length > 0) {
      return customerAddress.services.reduce(
        (total, item) =>
    ...
    const getAppointments = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/appointments/${userid.id}`,
          {
            headers: {
  ...
  const toggleService = (item) => {
    ...
    let isItemPresent = customerAddress.services.some(
      (ele) => ele.title == item.name
    );
    if (isItemPresent) {
      let updateList = customerAddress.services.filter(
        (ele) => ele.title != item.name
      );
      setCustomerAddress((prev) => ({
        ...prev,
  ...
  const bookAppointment = async () => {
    ...
    if (customerAddress.location === "Bureau") {
      if (!phone) {
            ...
            <View key={index}>
              ...
              >
                ...
                <View>
                  ...
                  {customerAddress?.services?.length > 0 &&
                    customerAddress.services.map((serviceItem, id) => (
                      <View key={id} style={styles.serviceItem}>
                        ...
                        <View style={styles.quantity}>
                          <TouchableOpacity
                            onPress={() => decrementQuantity(serviceItem.title)}
                          >
                            <Icon
                          ...
                          <TouchableOpacity
                            onPress={() => incrementQuantity(serviceItem.title)}
                          >
                            <Icon
                  ...
                  <View>
                    ...
                    <View style={styles.dateContainer}>
                      <Text style={styles.dateText}>
                        {customerAddress.appointment_date}
                      </Text>
                  ...
                  <View style={styles.totalPrice}>
                    ...
                    {service[0]?.payment_mode != "online" ? (
                      ...
                      <Text style={styles.price}>
                        €{customerAddress.total_price || 0.0}
                      </Text>
                    )}
                  </View>
                </View>
                <View style={styles.customerFormContainer}>
                  ...
                  <TextInput
                    ...
                    onPress={resetError}
                  />
                  {customerAddress?.location !== "Bureau" && (
                    <>
                  ...
                  {service[0]?.payment_mode != "online" && (
                    <>
                      <View style={styles.radioContainer}>
                        <Text style={styles.customerLabelLoc}>
                          Lieu de réservation du service?
                        </Text>
                        <TouchableOpacity
                        ...
                        >
                          <View
                            style={[
                              styles.radioCircle,
                              customerAddress.location === "Maison" &&
                      ...
                      <Text style={styles.customerLabelLoc}>
                        Délai d'annulation de la commande:
                      </Text>
                      ...
                      <TextInput
                        ...
                        onChangeText={(value) =>
                          handleChange("cancelComment", value)
                        }
                        multiline
                        onPress={resetError}
                      />
                ...
                {error && (
                  <Text style={styles.errorMessage}>
                    {showError
                      ? showError
                      : "Veuillez remplir les détails ci-dessus"}
                  </Text>
                )}
                    ...
                    <ScrollView style={styles.scrollView}>
                      {item.services.map((service, id) => (
                        <View key={id} style={styles.serviceItem}>
                          ...
                          <Switch
                            value={customerAddress.services?.some(
                              (ele) => ele.title === service?.name
                            )}
